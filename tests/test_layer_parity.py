...
def test_rope(hf_model_and_tokenizer):
    ...
    k_np = np.random.randn(batch_size, seq_len, config.model.num_key_value_heads, head_dim).astype(
        np.float32
    )
...
def test_linear_attention_chunked(hf_model_and_tokenizer):
    ...
    print("  Layer type: Gated DeltaNet")
    ...
    q_np = np.random.randn(batch_size, num_heads, seq_len, config.model.linear_key_head_dim).astype(
        np.float32
    )
    k_np = np.random.randn(batch_size, num_heads, seq_len, config.model.linear_key_head_dim).astype(
        np.float32
    )
