...
def test_paged_attention_vs_standard():
    ...
    query_input = jnp.array(
        np.random.randn(batch_size, seq_len, num_heads, head_dim).astype(np.float32)
    )
    key_input = jnp.array(
        np.random.randn(batch_size, seq_len, num_kv_heads, head_dim).astype(np.float32)
    )
    value_input = jnp.array(
        np.random.randn(batch_size, seq_len, num_kv_heads, head_dim).astype(np.float32)
    )
    ...
    print(f"  Standard output shape: {standard_output.shape}")
    print("  KV cache structure initialized")
    ...
    assert kv_cache.k_cache.shape[1] == 64
...
def test_linear_attention_multichunk_matches_recurrent():
    ...
    np.testing.assert_allclose(
        np.array(chunked_output), np.array(recurrent_output), rtol=1e-5, atol=1e-5
    )
...
def test_multi_layer_linear_attention_states():
    ...
    kv_cache = init_linear_attention_states(kv_cache, config.model, batch_size, dtype=jnp.float32)
...
def test_paged_attention_non_identity_blocks():
    ...
    print("  Position: 25")
    print(
        f"  Expected slot seq 0: {expected_slot_seq0} (block {block_table[0, 1]}, slot {25 % block_size})"
    )
    print(
        f"  Expected slot seq 1: {expected_slot_seq1} (block {block_table[1, 1]}, slot {25 % block_size})"
    )
    ...
    assert kv_cache.k_cache.shape[1] == num_blocks
...
def test_decode_attention_long_sequences():
    ...
    query = jnp.ones(
        (batch_size, 1, config.model.num_attention_heads, config.model.head_dim), dtype=jnp.float32
    )
...
def test_paged_decode_attention_independent_of_physical_cache_capacity():
    ...
    values = jnp.array(
        np.random.randn(batch_size, kv_len, num_kv_heads, head_dim).astype(np.float32)
    )
...
def test_masked_update_kv_cache_preserves_invalid_and_duplicate_slots():
    ...
    initial_k = jnp.arange(
        num_blocks * block_size * num_kv_heads * head_dim, dtype=jnp.float32
    ).reshape(
        1,
        num_blocks,
        block_size,
    ...
    initial_v = (
        jnp.arange(num_blocks * block_size * num_kv_heads * head_dim, dtype=jnp.float32) + 1_000.0
    ).reshape(
        1,
        num_blocks,
        block_size,
