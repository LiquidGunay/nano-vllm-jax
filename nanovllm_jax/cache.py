...
class KVCacheStorage:
    """Engine-owned physical KV arrays.

    The canonical implementation uses the teaching layout:
    [num_layers, num_blocks, block_size, num_kv_heads, head_dim].
    External kernels can use different physical layouts behind this boundary.
    """
...
def estimate_kv_cache_bytes(spec: KVCacheSpec) -> int:
    ...
    elements_per_cache = (
        spec.num_layers * spec.num_blocks * spec.block_size * spec.num_kv_heads * spec.head_dim
    )
...
jax.tree_util.register_pytree_node(KVCacheState, _kv_cache_state_flatten, _kv_cache_state_unflatten)
...
def _resolve_kv_layer(
    ...
    if cache.ndim == 5:
        if layer_idx >= cache.shape[0]:
            raise ValueError(f"layer_idx {layer_idx} is out of bounds for {cache.shape[0]} layers")
...
def update_kv_cache(
    ...
    k_cache_flat = (
        k_cache_layer if cache_is_flat else k_cache_layer.reshape(-1, num_kv_heads, head_dim)
    )
    v_cache_flat = (
        v_cache_layer if cache_is_flat else v_cache_layer.reshape(-1, num_kv_heads, head_dim)
    )
...
def paged_attention(
    ...
    k_flat = k_cache_layer if cache_is_flat else k_cache_layer.reshape(-1, num_kv_heads, head_dim)
    v_flat = v_cache_layer if cache_is_flat else v_cache_layer.reshape(-1, num_kv_heads, head_dim)
...
def paged_attention_prefill_packed(
    ...
    out_rows = jax.nn.dot_product_attention(
        query_rows.reshape(row_count, row_query_len, num_heads, head_dim).astype(k_gathered.dtype),
        k_gathered,
        v_gathered,
        mask=attn_mask[:, None, :, :],
        scale=scale,
    ...
    out = out.at[safe_token_indices.reshape(-1)].add(out_rows.reshape(-1, num_heads * head_dim))
...
def paged_attention_decode(
    ...
    query_len > 1 by applying an explicit causal mask from runtime query
    positions while keeping the materialized KV window static from the
    block-table width.
    ...
    # Use gather here because it is stable across supported JAX backends.
    ...
    out = jnp.einsum("btkgs,bskgd->btkgd", attn_weights, v_gathered_grouped)
    out = out.reshape(batch, query_len, num_heads, head_dim).reshape(
