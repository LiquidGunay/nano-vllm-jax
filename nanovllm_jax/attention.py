...
from nanovllm_jax.layers import apply_rope
...
def full_attention_block(
    ...
    if is_prefill and attention_metadata is not None:
        if attention_metadata.token_row_ids is not None:
            valid_token_mask = jnp.arange(seq_len, dtype=jnp.int32)[
                None, :
    ...
    q_gate_reshaped = q_gate.reshape(
        batch,
        seq_len,
        config.model.num_attention_heads,
        2 * config.model.head_dim,
    )
    query, gate = jnp.split(q_gate_reshaped, 2, axis=-1)
    gate = gate.reshape(batch, seq_len, -1)

    k = k_raw.reshape(batch, seq_len, config.model.num_key_value_heads, config.model.head_dim)
    v = v_raw.reshape(batch, seq_len, config.model.num_key_value_heads, config.model.head_dim)
    ...
    if kv_cache_state is not None:
        ...
        attn = nn.softmax(
            jnp.einsum("bhtd,bhsd->bhts", query, k) / jnp.sqrt(config.model.head_dim)
            + mask[None, None, :, :].astype(query.dtype),
            -1,
        )
        out = (
            jnp.einsum("bhts,bhsd->bhtd", attn, v).transpose(0, 2, 1, 3).reshape(batch, seq_len, -1)
        )
