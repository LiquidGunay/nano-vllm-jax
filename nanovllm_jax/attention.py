...
def full_attention_block(
    ...
    def _proj(inp: jnp.ndarray, weight: jnp.ndarray) -> jnp.ndarray:
        ...
        if is_prefill:
            return _compact_prefill_dot_if_enabled(
                ...
                valid_token_mask,
                compact_prefill_tokens,
                enabled=_enable_compact_prefill_full_attn_proj(config.kernels),
            )
    ...
    attn_out_dim = config.model.num_attention_heads * config.model.head_dim
    ...
    if force_width1_full_attn:
        ...
        for t in range(seq_len):
            ...
            q_gate_t = q_gate_t.reshape(batch, 1, config.model.num_attention_heads, 2 * config.model.head_dim)
            query_t, gate_t = jnp.split(q_gate_t, 2, axis=-1)
            k_t = jnp.dot(x_t.reshape(batch, hidden_t), params["k_proj"])[:, None, :].reshape(
                batch, 1, config.model.num_key_value_heads, config.model.head_dim
            )
            v_t = jnp.dot(x_t.reshape(batch, hidden_t), params["v_proj"])[:, None, :].reshape(
                batch, 1, config.model.num_key_value_heads, config.model.head_dim
            )

            query_t = rms_norm(query_t, params["q_norm"], config.model.rms_norm_eps).transpose(0, 2, 1, 3)
            k_t = rms_norm(k_t, params["k_norm"], config.model.rms_norm_eps).transpose(0, 2, 1, 3)
            ...
            query_t = apply_rope(
                ...
                config.model.partial_rotary_factor,
                layout="BHTD",
                mrope_section=config.model.mrope_section,
            )
            k_t = apply_rope(
                k_t,
                pos_t,
        ...
        if use_packed_decode_qkv or use_packed_prefill_qkv:
            ...
            k_end = q_gate_end + config.model.num_key_value_heads * config.model.head_dim
        ...
        q_gate_reshaped = q_gate.reshape(batch, seq_len, config.model.num_attention_heads, 2 * config.model.head_dim)
        query, gate = jnp.split(q_gate_reshaped, 2, axis=-1)
        gate = gate.reshape(batch, seq_len, -1)

        k = k_raw.reshape(
            batch, seq_len, config.model.num_key_value_heads, config.model.head_dim
        )
        v = v_raw.reshape(
            batch, seq_len, config.model.num_key_value_heads, config.model.head_dim
        )
        ...
        query = apply_rope(query, positions, config.model.head_dim, config.model.rope_theta, config.model.partial_rotary_factor, layout="BHTD", mrope_section=config.model.mrope_section)
        k = apply_rope(k, positions, config.model.head_dim, config.model.rope_theta, config.model.partial_rotary_factor, layout="BHTD", mrope_section=config.model.mrope_section)
    ...
    if kv_cache_state is not None:
        ...
        attn = nn.softmax(jnp.einsum("bhtd,bhsd->bhts", query, k) / jnp.sqrt(config.model.head_dim) + mask[None, None, :, :].astype(query.dtype), -1)
